  ...
  <div>
    ...
    <div class="nav-bar">
      ...
      <el-breadcrumb>
        <el-breadcrumb-item v-for="item of breadcrumbList" :key="item.path">
          <span v-if="item.redirect">{{ item.meta.menuName }}</span>
      ...
      <div class="right-menu">
        ...
        <el-dropdown @command="handleCommand">
          <el-avatar :size="40" :src="this.$store.state.user.user.avatar" />
    ...
    <div class="tabs-view-container">
      <div class="tabs-wrapper">
        <span
          :class="isActive(item)"
          v-for="item of this.$store.state.menu.tabList"
          :key="item.path"
          @click="goTo(item)"
        >
          {{ item.name }}
...
export default {
  ...
  methods: {
    ...
    removeTab(tab) {
      //删除标签
      this.$store.commit("menu/removeTab", tab);
      //如果删除的是当前页则返回上一标签页
    ...
    trigger() {
      this.$store.commit("menu/trigger");
    },
    ...
    closeAllTab() {
      this.$store.commit("menu/resetTab");
  ...
  computed: {
    ...
    isFold() {
      return this.$store.state.menu.collapse ? "el-icon-s-unfold" : "el-icon-s-fold";
    }
  }
};
</script>
